  ...
  function codigoCat(catstr) {
	...
	function listarProductos(productos) {
	  ...
	  for(nfila=0;nfila<num;nfila++) {
		...
		Accion[nfila].innerHTML= "<button id = 'eliminarBtn' class='btn btn-danger'>Delete</button>"
		Accion[nfila].firstChild.setAttribute("onclick","eliminarProductos('"+productos[nfila].id+"');");
		}
	}

function obtenerProductos() {
	  fetch('https://retoolapi.dev/osaNow/fakestore')
            .then(res=>res.json())
            .then(data=>{
				productos=data;
				productos.forEach(
...
function eliminarProductos(iden) {
	fetch('https://retoolapi.dev/osaNow/fakestore/'+iden, { method: "DELETE" })
